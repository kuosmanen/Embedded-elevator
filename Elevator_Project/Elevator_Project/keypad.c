...
/**
 * Checks a potential key press input and inspects whether it is a byte or not.
 * 
 * Returns 1 if a keypress has been detected. 0 if not.
 */
...
uint8_t KEYPAD_GetKeyNonBlocking(void)
{
	...
	// Providing a delay, after which a key input is converted from a raw
	// input into a readable value.
...
static uint8_t keypad_ScanKey(void)
{
    ...
    for (row = KEYSCAN_INITIAL; row < KEYSCAN_UPPER_CAP; row++) {
        M_ROW = scan_code;
        DELAY_ms(1);
